...
export default function Album() {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-rose-500/20 bg-rose-500/10 p-8">
          <p className="text-rose-400">Album not found or failed to load.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Album Header */}
      <div className="rounded-3xl border border-slate-700 bg-gradient-to-br from-slate-900 to-slate-950 p-8">
        <div className="flex flex-col gap-8 md:flex-row md:items-end">
          {/* Album Cover */}
          <div className="h-56 w-56 flex-shrink-0 overflow-hidden rounded-2xl bg-gradient-to-br from-orange-600 to-red-600 shadow-2xl shadow-black/50">
            ...
            <div className="space-y-2">
              {album.artist && (
                <p className="text-lg text-slate-400">
                  By <span className="font-semibold text-white">{album.artist}</span>
                </p>
              )}
              {album.release_date && (
        ...
        <section className="space-y-6">
          <div>
            ...
            <p className="mt-2 text-slate-400">
              {albumTracks.length} tracks on this album
            </p>
          </div>
      ...
      {albumTracks.length === 0 && (
        <div className="rounded-2xl border border-slate-700 bg-slate-950 p-12 text-center">
          ...
          <p className="text-slate-400">No tracks on this album</p>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="relative -mt-24 space-y-6 px-8 pb-8">
          ...
          <div className="flex items-end gap-6">
            ...
            <div className="flex-1 pb-4">
              ...
              <h1 className="text-5xl font-bold text-white">{album.name}</h1>
              {album.artist && (
                <p className="mt-4 text-xl text-slate-300">by {album.artist}</p>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-6 border-t border-slate-800 pt-6">
            ...
            <div>
              ...
              <p className="text-3xl font-bold text-white">{releaseYear}</p>
            </div>

            {/* Songs Count */}
            <div>
              <p className="text-sm text-slate-400">Tracks</p>
              <p className="text-3xl font-bold text-white">{albumTracks.length}</p>
            </div>
      ...
      {albumTracks.length > 0 && (
        <section className="space-y-6">
          <div>
            <h2 className="text-3xl font-bold text-white">Tracklist</h2>
            <p className="mt-2 text-slate-400">{albumTracks.length} songs</p>
          </div>
      ...
      {albumTracks.length === 0 && (
        <div className="rounded-2xl border border-slate-700 bg-slate-950 p-12 text-center">
          ...
          <p className="text-slate-400">No tracks available for this album</p>
        </div>
      )}
    </div>
  )
